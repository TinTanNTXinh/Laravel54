...
export class ButtonProductComponent implements OnInit, IComponent {
    ...
    updateOne(): void {
        this.httpClientService.put(this.prefix_url, {"tray_product": this.tray_product_update}).subscribe(
            (success: any) => {
                ...
                this.utilitiesService.toggleModal('modal-update');
            },
            (error: any) => {
    ...
    validateOne(): boolean {
        ...
        if (this.tray_product.arr_tray_id.length == 0) {
            this.utilitiesService.showToastr('warning', 'Vui lòng chọn ít nhất 1 mâm.');
            flag = false;
        }
        if (this.tray_product.product_id == 0) {
            this.utilitiesService.showToastr('warning', 'Vui lòng chọn 1 sản phẩm.');
            flag = false;
        }
        return flag;
    }
    ...
    actionCrud(obj: any): void {
        switch (obj.mode) {
            case 'add':
                ...
                break;
            case 'edit':
                this.utilitiesService.getElementById('btn-show-modal1').click();
