...
export class ReportDistributorComponent implements OnInit, IComponent {
    ...
    public downloadFile(mode: string) {
        ...
        this.httpClientService.get(`${this.prefix_url}/${url}`, 'text')
            .subscribe(
                (success: any) => {
                    this.utilitiesService.downloadFile(success, `BaoCaoDaiLy_${subfix_filename}.csv`, 'text/csv');
                },
                (error: any) => {
                    this.utilitiesService.showToastr('error');
                }
